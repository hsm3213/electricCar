(function ($, window) {
  ...
  window.selectMenu = {
    init: function () {
      ...
      customSelect._drawMenu = function () {
        ...
        // 셀렉스 박스 생성 시 dimmed 영역 포함
        // 디자인에 맞게 변경
        this.menuWrap.after('<div class="dimmed"></div>');
        this.menuWrap.prepend(
          '<h3 class="tit_select">' + this.element.data("title") + "</h3>"
        );
        this.menuWrap.append(
          '<button type="button" class="close">닫기</button>'
        );
        this.menu.wrap('<div class="tselect_scroll"></div>');
  ...
  $(function () {
    ...
    $(document).on("click", ".select_wrap .close", function () {
      $(this)
        .closest(".select_wrap")
        .find("select")
